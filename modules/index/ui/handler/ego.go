...
import (
	...
	"github.com/infinitbyte/gopa/modules/index/ui/config"
	"html"
	"io"
	"net/http"
	"strings"
...
func Search(w io.Writer, r *http.Request, q string, filter string, from int, size int, config *config.UIConfig, response *index.SearchResponse) error {
	_, _ = io.WriteString(w, "\n")
	_, _ = io.WriteString(w, "\n")
	_, _ = io.WriteString(w, "\n")
	_, _ = io.WriteString(w, "\n")
	...
	if len(response.Aggregations) > 0 {
		...
		for k, v := range response.Aggregations {
			if len(v.Buckets) > 0 {
				...
				_, _ = io.WriteString(w, html.EscapeString(fmt.Sprint(getBucketLabel(k))))
				...
				for _, bkt := range v.Buckets {
					...
					_, _ = io.WriteString(w, "\n                                <li><a href=")
					_, _ = fmt.Fprint(w, fmt.Sprintf("?q=%s&filter=%s|%v&from=0", q, getBucketKey(k), util.UrlEncode(key)))
					_, _ = io.WriteString(w, " >")
