...
func (h *UserUI) GetSnapshotPayloadAction(w http.ResponseWriter, req *http.Request, ps httprouter.Params) {
	...
	if len(bytes) > 0 {
		...
		if util.ContainStr(snapshot.ContentType, "text/html") {
			...
			h.Write(w, []byte(fmt.Sprintf(`var dom=document.createElement("div");dom.innerHTML='<div style="height:20px;padding-left:10px;position: absolute:top:100px;background:#ebebeb;font-size: 12px;text-align:center;"><img border=0 style="float:left;height:20px" src="%s"><span style="font-size: 12px;">Generated by Gopa, %v, <a href="%v">%v</a></span></div>';var first=document.body.firstChild; document.body.insertBefore(dom,first);`, h.Config.SiteLogo, snapshot.Created, snapshot.Url, snapshot.Url)))
