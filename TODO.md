...
修复下载页面路径，如果页面路径是/绝对路径，需要修复为相对路径，否则本地文件无法正常预览

抓取深度控制，pending_fetch保存上级url和深度信息

支持本地文件url路径（file:///），单独hunter处理

处理列表页，默认分解参数并保存到文件名中，参数名排序

使用DHT分布式Hash来去重和保存数据

使用JSON存储配置文件和元数据

默认本地存储，提供一键重建索引到es提供搜索功能

目录控制文件数目和文件大小，参考weedfs

存储使用https://github.com/syndtr/goleveldb 来保存元数据和任务信息
  key: `根域名:Hash(URL)`
  value: JSON格式 `{
    "root_domain"
    "domain"
...
goroutine添加buffer,shutdown的时候关闭chan,并处理关闭事件 [done]
...
文件列式存储,实现浏览接口,按域名分开存放,设置多级目录,方便批量数据处理

Blotdb: 域名下主键自增,按目录进行prefix遍历
...
URL进来就开始跟踪状态,抓取、解析 etc

URL入库时进行DHT查询去重,抓取到解析在一个节点上进行,不用分布式

URL检测,单个chan,批量处理,性能
